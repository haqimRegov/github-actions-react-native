...
const setBadge = (count: string | number) => {
  const badgeCount = typeof count === "string" ? parseInt(count, 10) : count;
  PushNotificationIOS.setApplicationIconBadgeNumber(badgeCount);
};

const localNotification = async ({ data, notification }: IPushNotification) => {
  PushNotification.localNotification({
    ...
    // shortcutId: "shortcut-id", // (optional) If this notification is duplicate of a Launcher shortcut, sets the id of the shortcut, in case the Launcher wants to hide the shortcut, default undefined
    ...
    // id: 0, // (optional) Valid unique 32 bit integer specified as string. default: Auto generated Unique ID
