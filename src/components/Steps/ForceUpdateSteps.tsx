...
interface ForceUpdateStepsProps {
  activeContent?: IForceUpdateContentItem | IForceUpdate;
  activeSection: number;
  disabledSteps?: TypeForceUpdateKey[];
  disableNextSteps?: boolean;
  finishedSteps?: TypeForceUpdateKey[];
  handleBackToDashboard: () => void;
  handleContentChange: (item: IForceUpdateContentItem | IForceUpdate) => void;
  RenderContent: (props: ForceUpdateStepsContentProps) => JSX.Element;
  setActiveContent: (content: IForceUpdateContentItem | IForceUpdate) => void;
  setActiveSection: (section: number) => void;
  setFinishedStep: (step: TypeForceUpdateKey[]) => void;
  steps: IForceUpdate[];
}

export const ForceUpdateSteps: FunctionComponent<ForceUpdateStepsProps> = ({
  activeContent,
  activeSection,
  disableNextSteps,
  disabledSteps,
  ...
  const accordionContent = (step: IForceUpdate) => {
  ...
  const handleNextStep = (nextRoute: string) => {
    let newIndex = 0;
    const parentRoute = steps.filter((item: IForceUpdate, index: number) => {
    ...
    const childRoute = steps.map((item: IForceUpdate) => {
      if (item.content !== undefined) {
        return item.content.filter((contentItem: IForceUpdateContentItem) => {
    ...
    const singleChildRoute = childRoute.filter((content: IForceUpdateContentItem[], index: number) => {
      if (content.length !== 0) {
        newIndex = index;
      }
